...
# Define the DAG
with DAG(
    dag_id="cp_dbt_terminal_kpi_return_feed_v1",
    ...
    wait_task = ExternalTaskSensor(
        task_id="wait_for_task_cp_dbt_termian_kpi_refined_task",
        external_dag_id="cp_dbt_terminal_kpi_refined_v1",
        external_task_id="cp_dbt_terminal_kpi_refined_task",
        mode="poke",
        soft_fail=True, 
    ...
    dbt_run = BashOperator(
        task_id="cp_dbt_termian_kpi_return_feed_task",
        bash_command="""
        TS="{{ ts_nodash }}" 
        TEMP_DIR="/tmp/cp_dbt_terminal_kpi_return_feed_task_$TS"
        ...
        {DBT_EXECUTABLE} run --profiles-dir {DBT_PROFILE_PATH} --select tag:return_feed 
        """.replace("{DBT_EXECUTABLE}", DBT_EXECUTABLE)
           .replace("{DBT_PROFILE_PATH}",  DBT_PROFILE_PATH),
        env=DBT_ENV_VARS | {"PROCESSING_DATETIME__RETURN_FEED": "{{ ds }}"},
        )
