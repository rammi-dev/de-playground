...
# Define the DAG
with DAG(
    dag_id="cp_dbt_termianal_kpi_asset_v1",
    ...
    DBT_ENV_PARTITION = {
        "PROCESSING_DATETIME__SILVER": "{{ prev_execution_date }}",
    }

    vars = DBT_ENV_VARS | DBT_ENV_PARTITION

    wait_task = ExternalTaskSensor(
        task_id="wait_for_task_cp_dbt_termian_kpi_raw_task",
        external_dag_id="cp_dbt_termianal_kpi_raw_v1",
        external_task_id="cp_dbt_termianal_kpi_raw_task",
        mode="poke",
        timeout=1800,  # Wait up to 1 hour
    )

    # Define the dbt run task
    dbt_run = BashOperator(
        task_id="cp_dbt_termian_kpi_asset_task",
        bash_command="""
        TS="{{ ts_nodash }}" 
        TEMP_DIR="/tmp/cp_dbt_termian_kpi_asset_task_$TS"
